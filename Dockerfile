...
# countmut >= 0.2.4: run_countmut emits the per-strand 2-group conversion
...
# 0.2.4 adds the strand-aware `base` token and --target-base (correct '-'
# strand m6A A-sites) and fixes the --target-base default to no-filter.
...
RUN python${PYTHON_VERSION_FOR_APP} -m venv ${VENV_PATH} && \
    uv pip install --python ${VENV_PATH}/bin/python --no-cache \
        ...
        countmut==0.2.4 coralsnake==0.2.1 prismalign==0.3.6 \
