...
# countmut >= 0.2.5: run_countmut emits the per-strand 2-group conversion
...
# 0.2.5 evaluates -p PER STRAND, so `base == 'A'` keeps only the A-site
# strand and drops the spurious complement-strand rows (correct '-'-strand
# m6A A-sites) without --target-base.
