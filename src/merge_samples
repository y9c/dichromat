...
def merge_to_parquet(files, names, requires, output_file, min_depth=3):
    ...
    # min_depth uses the raw per-sample depth columns (pre-project) - polars
    # filtered here too (after the first optional join, before renaming).
    final = base
    if any(r == 0 for _f, _n, r in ordered) and len(ordered) > 1:
        dlist = ", ".join(f'"{n}__D"' for _f, n, _r in ordered)
        final = final.filter(f"GREATEST({dlist}) >= {int(min_depth)}")
    final = final.project(", ".join(exprs)).order('Chrom, Pos, Strand')

    out = str(Path(output_file).resolve())
    Path(output_file).parent.mkdir(parents=True, exist_ok=True)
    final.write_parquet(out)
    logging.info("Merged %d samples -> %s (%d rows)",
                 len(ordered), output_file, len(final.fetchall()))
