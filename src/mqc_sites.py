...
def main():
    ...
    if args.sites_file and os.path.exists(args.sites_file):
        try:
            ...
            ratio_mids = (ratio_bins[:-1] + ratio_bins[1:]) / 2
            ...
            depth_labels = [f"{int(m)}" for m in depth_mids]
            ...
            for d_col in depth_cols:
                ...
                # Use explicit suffixes to prevent sample merging confusion
                plot_name = f"{sample}"

                # --- Ratio Distribution ---
                r_counts, _ = np.histogram(ratios, bins=ratio_bins)
                r_row = {'Sample': plot_name}
