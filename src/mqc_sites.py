...
def main():
    ...
    # 1. Motif Heatmap (Converted to % with higher precision)
    all_dfs = []
    if args.motif_files:
    ...
    if args.sites_file and os.path.exists(args.sites_file):
        try:
            ...
            # Bins for Ratio (Line plot, linear)
            ratio_bins = np.linspace(0, 1, 51)
            ratio_labels = [f"{ratio_bins[i]:.2f}" for i in range(len(ratio_bins)-1)]
            
            # Bins for Depth (Line plot, log scale)
            max_depth = 1000
            ...
            depth_labels = [f"{int(depth_bins[i])}" for i in range(len(depth_bins)-1)]
            ...
            for d_col in depth_cols:
                ...
                # --- Ratio Distribution ---
                r_counts, _ = np.histogram(ratios.to_numpy(), bins=ratio_bins)
                r_row = {'Sample': sample}
                for label, count in zip(ratio_labels, r_counts):
                    r_row[label] = int(count)
                ratio_dist_data.append(r_row)

                # --- Depth Distribution ---
                d_counts, _ = np.histogram(depths.to_numpy(), bins=depth_bins)
                d_row = {'Sample': sample}
                for label, count in zip(depth_labels, d_counts):
                    d_row[label] = int(count)
                depth_dist_data.append(d_row)
            ...
            if ratio_dist_data:
                df_ratio_dist = pl.DataFrame(ratio_dist_data)
                header_ratio = [
                    "# id: site_ratio_dist", 
                    "# section_name: 'Site Conversion Ratio Distribution'", 
                    "# description: 'Frequency of conversion ratios across all detected sites.'", 
                    "# plot_type: 'line'", 
                    "# pconfig:",
            ...
            if depth_dist_data:
                df_depth_dist = pl.DataFrame(depth_dist_data)
                header_depth = [
                    "# id: site_depth_dist", 
                    "# section_name: 'Site Depth Distribution'", 
                    "# description: 'Distribution of sequencing depth across all detected sites (log scale bins).'", 
                    "# plot_type: 'line'", 
                    "# pconfig:",
